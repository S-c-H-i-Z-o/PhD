...
A standard dataset metadata model should contain information about:

\begin{itemize}
  \item \textbf{Resources}: the distributable parts holding the actual raw data, they come in various formats (JSON, XML, RDF, etc.) and can be downloaded or accessed directly (REST API, SPARQL endpoint).
  \item \textbf{Tags}:
  \item \textbf{Groups}:
  \item \textbf{Organizations}:
\end{itemize}

Metadata models vary across portals and can be of different levels:

\begin{itemize}
  \item \textbf{Discovery metadata}:
  \item \textbf{Exploration metadata}:
  \item \textbf{Exploitation metadata}:
\end{itemize}
