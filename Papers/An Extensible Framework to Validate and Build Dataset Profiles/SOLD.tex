...
A total of 334 resources (31.27\%) URLs were not reachable, thus affecting highly the availability of these datasets. CKAN resources can be of various predefined types $(file, file.upload, api, visualization, code and documentation)$. Figure \ref{fig:5} shows the breakdown of these unreachable resources according to their types, where 211 (63.17\%) did not have valid \texttt{resource\_type}, 112 (33.53\%) were files, 8 (2.39\%) and one (0.029\%) metadata, example and documentation types.\\

\begin{figure}

\parbox{7cm}{\hspace*{-.2in}
\includegraphics[width=.95\linewidth]{access_information.png}
\captionsetup{textfont=small,singlelinecheck=off,justification=raggedright}
\caption{Access information error \% by field}
\label{fig:4}}
\qquad
\begin{minipage}{7cm}\hspace*{-.6in}
\includegraphics[width=.95\linewidth]{unreachable_type.png}
\captionsetup{textfont=small,singlelinecheck=off,justification=raggedright}
\caption{Unreachable URLs by format}
\label{fig:5}
\end{minipage}
...
\subsection{Provenance information} Provenance information is mainly computed automatically by the data portal \texttt{(revision\_id, metadata\_created, metadata\_modified, revision\_timestamp, revision\_id)}, only the \texttt{version} field was missing for 60.23\% of the datasets.
