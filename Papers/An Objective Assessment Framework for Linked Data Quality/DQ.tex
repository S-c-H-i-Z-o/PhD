...
Data quality assessment is the process of evaluating if a piece of data meets the consumers need in a specific use case \cite{Bizer:2009:QIF:1482178.1482280}. The dimensionality of data quality makes it dependent on the task, thus the users requirements. In this paper, we propose a comprehensive objective framework to evaluate the quality of Linked Data sources. The framework helps on one hand data owners to rate the quality of their dataset and get some hints on possible improvements, and on the other hand data consumers to choose their data sources from a ranked set. The aim of this paper is to provide researchers and practitioners with a comprehensive understanding of the objective issues surrounding Linked Data quality.

The framework we propose is based on a refinement of the data quality principles proposed in our previous work \cite{assaf2012} and those in \cite{Framework2012}. Some attributes have been grouped for more detailed quality assessments. We have also extended our framework by adding for each attribute a set of objective indicators. These indicators are measures that provide users with quality performance. Those indicators should be measurable automatically by tools regardless of the task. We surveyed the landscape of Linked Data quality tools to discover that they only cover a subset of the proposed quality indicators. We further present an extensible quality measurement tool and evaluate it against the LOD cloud. The results demonstrate that the general quality of LOD cloud needs more attention as most of the datasets suffer from various quality issues.
...
In \cite{DBLP:conf/i-semantics/RulaZ14}, authors propose a methodology for assessing Linked Data quality by analyzing the use case details provided by the user se cas. The methodology \\
