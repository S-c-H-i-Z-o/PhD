...
This paper is structured as follows: Section 2 presents the various quality assessment methodologies and frameworks, Section 3 presents our framework (objective quality measures and indicators); Section 4 presents our tool for evaluating objective Linked Data quality indicators; Section 5 presents our evaluation and experiments running this tool on LOD; Section 6 reviews the existing tools and framework in the Linked Open Data quality landscape; Section 7 presents concluding remarks and identifies future work.
...
\hline
\hline
\multirow{7}{*}{Comprehensibility} & \multirow{4}{*}{Dataset Level} & QI.37 & Existence of at least one exemplary RDF file \cite{Framework2012}\tabularnewline
\cline{3-4}
...
\cline{3-4}
 ...
 \cline{3-4}
 &  & QI.46 & Usage of a versioning\tabularnewline
\hline
\hline
...
For this paper, we have extended the framework with a new quality module to measure datasets quality. We have implemented 7 submodules that will check various dataset quality indicators. Various additional quality measures can be easily plugged in/out.\\
...
\hline
\hline
QI.1 & Check if there is a valid metadata file by issuing a \texttt{package\_show} request to the CKAN API\tabularnewline
\hline
QI.2 & Check if the \texttt{format} field for the dataset resources is defined and valid\tabularnewline
\hline
QI.3 & Check the \texttt{resource\_type} field with the following possible values \texttt{file, file.upload, api, visualization, code, documentation}\tabularnewline
\hline
QI.4 & Check the resources \texttt{format} field for \texttt{meta/void} value\tabularnewline
\hline
QI.5 & Check the resources \texttt{size} or the \texttt{triples} extras fields\tabularnewline
\hline
QI.6 & Check the \texttt{format} and \texttt{mimetype} fields for resources\tabularnewline
\hline
QI.7 & Check if the dataset has a \texttt{topic} tag and if it is part of a valid group in CKAN\tabularnewline
\hline
QI.9 & Check if the dataset and all its resources have has a valid URI \tabularnewline
\hline
QI.18 & Check if there is a dereferencable resource with a description containing string \textit{dump}\tabularnewline
\hline
QI.19 & Check if there is a dereferencable resource with \texttt{resource\_type} of type \texttt{api}\tabularnewline
\hline
QI.20 & Check if all the links assigned to the dataset and its resources are dereferencable\tabularnewline
\hline
QI.21 & Check if the dataset contains valid \texttt{license\_id} and \texttt{license\_title}\tabularnewline
\hline
QI.22 & Check if the \texttt{license\_url} is dereferencable\tabularnewline
\hline
QI.24 & Check if the dataset and its resources contain the following metadata fields \texttt{metadata\_created, metadata\_modified, revision\_timestamp, cache\_last\_updated}\tabularnewline
\hline
QI.25 & Check if the \texttt{content-type} extracted from the a valid HTTP request is equal to the corresponding \texttt{mimetype} field.\tabularnewline
\hline
QI.26 & Check if the \texttt{content-length} extracted from the a valid HTTP request is equal to the corresponding \texttt{size} field.\tabularnewline
\hline
QI.28,29 & Check that all the links are valid HTTP scheme URIs\tabularnewline
\hline
QI.37 & Check if there is at least one resource with a \texttt{format} value corresponding to one of \texttt{example/rdf+xml, example/turtle, example/ntriples, example/x-quads, example/rdfa, example/x-trig}\tabularnewline
\hline
...
\hline
QI.40 & Check if the dataset contain valid \texttt{author\_email} or \texttt{maintainer\_email} fields\tabularnewline
\hline
QI.44 & Check if the dataset and its resources contain provenance metadata \texttt{maintainer, owner\_org, organization, author, maintainer\_email, author\_email}\tabularnewline
\hline
QI.46 & Check if the dataset contain and its resources contain versioning information \texttt{version, revision\_id} \tabularnewline
\hline
\end{tabular}
}
\end{center}

In our framework, we have presented 30 objective quality indicators related to dataset and links quality. Our tool is able to assess and score 23 of them. We excluded security related quality indicators as LOD cloud group members should not restrict access to their datasets. Our tool is not able to check the existence of information about the kind and number of used vocabularies (QI.8), license permissions, copyrights and attributes (QI.23), exemplary SPARQL query (QI.38), usage of provenance vocabulary (QI.45) and is not able to check the dataset for syntactic errors (QI.27).\\These shortcomings are mainly due to the limitations in the CKAN dataset model. However, syntactic checkers and additional modules to examine vocabularies usage could be easily integrated in our framework to fix QI.27, QI.8 and QI.45. Our metadata quality profiler presented as part of our framework can fix QI.23 as we have manually created a mapping file standardizing the set of possible license names and their information\footnote{https://github.com/ahmadassaf/opendata-checker/blob/master/util/licenseMappings.json}. We have also used the open source and knowledge license information\footnote{https://github.com/okfn/licenses} to normalize license information and add extra metadata like the domain, maintainer and open data conformance.

\subsection{Quality Score Calculation}

A CKAN dataset metadata describes three main sections in addition to the core dataset's properties. Those are the groups, tags and resources. Each section contains a set of metadata corresponding to one or more metadata type.
...
\begin{lstlisting}
   ...
   ============================================================================
   completeness quality Score      :   77.76%
   availability quality Score      :   74.50%
   licensing quality Score         :   60.88%
   freshness quality Score         :   45.90%
   correctness quality Score       :   53.96%
   comprehensibility quality Score :   60.78%
   provenance quality Score        :   49.38%
   Average total quality Score     :   60.45%
   ============================================================================
                            Data Quality Problems
   ============================================================================
   Supports multiple serializations : 88.65%
   Has different data access points : 80.69%
   Uses datasets description vocabularies : 88.80%
   Existence of descriptions about its organization and categorization : 41.67%
   Existence of information about the kind and number of used vocabularies : 0%
   Existence of an RDF dump that can be downloaded by users : 84.17%
