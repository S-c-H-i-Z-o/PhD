...
\begin{figure}[ht!]
\centering
	\includegraphics[width=0.95\textwidth]{information_grouping.png}
...
The CKAN model controls the values to be used in describing some dataset properties. For example, the \texttt{resource\_type} property can have the values: file: direct accessible bitstream, file.upload: file uploaded to the CKAN FileStore\footnote{\url{http://docs.ckan.org/en/ckan-1.8/filestore.html}}, api, visualization, code: the actual source code or a reference to a code repository and documentation. To know the set of values in these fields we examined the models of several datasets with a tool we created called Roomba, see chapter~\ref{chapter:roomba}.
...
\lstset{basicstyle=\scriptsize, backgroundcolor=\color{white}, frame=single, caption= {Excerpt of the report for aggregating \textit{extras} field values on the LOD Cloud}, captionpos=b, label=lodcloud_extras_report}
\begin{lstlisting}
